...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (target) {
      ...
      await channel.bulkDelete(fitlered).then((messages) => {
        res.setDescription(
          `Succesfully deleted ${messages.size} messages from ${target}.`
        );
        interaction.reply({ embeds: [res], ephermal: true }); // use ephermal or not
      });
    } else {
      await channel.bulkDelete(amount, true).then((messages) => {
        ...
        interaction.reply({ embeds: [res], ephermal: true });
      });
    }
  },
};
